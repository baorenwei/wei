...
final class DecodeHandler extends Handler {
  ...
  public void handleMessage(Message message) {
    switch (message.what) {
      case Intents.Action.decode:
        //Logger.d(TAG, "Got decode message");
        decode((byte[]) message.obj, message.arg1, message.arg2);
        break;
      case Intents.Action.quit:
  ...
  private void decode(byte[] data, int width, int height) {
    ...
    if (rawResult != null) {
      ...
      Message message = Message.obtain(activity.getHandler(), Intents.Action.decode_succeeded, rawResult);
      ...
      Message message = Message.obtain(activity.getHandler(), Intents.Action.decode_failed);
      message.sendToTarget();
    }
  }
